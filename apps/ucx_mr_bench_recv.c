...
int main(int argc, char **argv)
{
  ...
  mr_bench_ctx.mr_ctx.server_addr = NULL;
  ...
  if (mr_bench_ctx.repeated_test == 0)
  {
    ...
    {
    case MR:
      ucx_mr_bench_recv_mr(&mr_bench_ctx);
      break;
    case SINGLE:
      for (int i = 0; i < NOF_RAILS; ++i)
      {
        ucx_mr_bench_recv_single(&mr_bench_ctx, i);
      }
      break;
    case SPLIT:;
      printf("Run split bench with %f split ratio, %d stages and %d rails\n", split_ratio, pipeline_stages, nof_rails);
      ucx_mr_bench_recv_split(&mr_bench_ctx, split_ratio, pipeline_stages);
      break;
    case TEST_SPLIT:
      ucx_mr_bench_test_recv_split(&mr_bench_ctx, split_ratio, pipeline_stages);
      break;
    default:
  ...
  else
  {
    ...
    strftime(filename, sizeof(filename), "./export/export_recv_%Y-%m-%d_%H:%M.txt", timenow);

    fptr = fopen(filename, "w");


    fprintf(fptr, "-----Repeated Benchmarks on Receiver Side-----\n");
    ...
    for (int k = 0; k < 10; ++k)
    {
      ...
      for (int j = 0; j <= mr_bench_ctx.repetitions; ++j)
      {
        ...
        {
        case MR:
          bw = ucx_mr_bench_recv_mr(&mr_bench_ctx);
          if (j != 0)
            fprintf(fptr, "|   MR%d   |  %s  |% 24e|% 11.2lf|   -   |   -  |\n", nof_rails, mem, nof_rails * ms, bw);
          break;
        case SINGLE:
          for (int i = 0; i < NOF_RAILS; ++i) {
            bw = ucx_mr_bench_recv_single(&mr_bench_ctx, i);
            if (j != 0)
        ...
        case SPLIT:
          ...
          {
            bw = ucx_mr_bench_recv_split(&mr_bench_ctx, i, p);
            if (j != 0)
